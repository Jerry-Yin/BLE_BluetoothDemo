...
//        }
    ...
    private void broadcastUpdate(final String action,
        ...
        if (data != null ) {
            ...
            Log.d(TAG, "broadcastUpdate() is running...    data = :" +data.toString());
        }
        sendBroadcast(intent);
    }
    ...
    /**
     * 向蓝牙中写入指令
     * @param characteristic 该特征值中已经通过 setValue(byte[] value) 设置了要发送的数据
     */
    ...
    public void writRXCharacteristic(String data){
        ...
        try {
            ...
            boolean status = mBluetoothGatt.writeCharacteristic(rxCharacteristic);
